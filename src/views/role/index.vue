        ...
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item label="名称">
              <a-input class="field-right" placeholder="请输入角色名称" v-decorator="[`name`]" />
            </a-form-item>
          </a-col>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				let res = await GetRolePageList(data)
				this.loading = false
			...
			async getOtherList() {
				...
				str = str.replace(/GroupData/g, 'children')
				str = str.replace(/Functionchildren/g, 'children')
				str = str.replace(/FunctionData/g, 'children')
				str = str.replace(/Name/g, 'title')
				str = str.replace(/ID/g, 'value')
				this.RoleList = JSON.parse(str)
			},
			addList() {
				this.text = {}
			...
			deleteList(text) {
				...
				this.$confirm({
					...
					async onOk() {
						let data = {}
						data.id = text.ID
						let res = await DoDeleteRole(data)
						if (res.data.code === 0) {
							that.getList()
