<template>
	<div class="all"  v-loading="loading">
		...
		<div class="option">
			<!-- <a-button  type="primary" class="btn" @click="addList">发起公告</a-button> -->
		</div>
		<div>
			<el-table ref="singleTable" :data="data" border highlight-current-row style="width: 100%">
				...
				<el-table-column prop="Content" label="公告内容" min-width="180" ></el-table-column>
				<el-table-column prop="AddUserName" label="发布人" min-width="80" ></el-table-column>
				<el-table-column prop="AddTime" label="发布时间" min-width="80" ></el-table-column>
				<el-table-column prop="IsEnable" label="操作" min-width="60" fixed="right">
					<template slot-scope="scope">
						<i  title="讨论" class="iconfont icon-liaotian" @click="talk(scope.row)"></i>
						<i title="修改" class="iconfont icon-editor1" style="color: #1890ff;" @click="update(scope.row)"></i>
						<i title="删除" class="iconfont icon-delete1" style="color: red;" @click="deletemember(scope.row)"></i>
					</template>
				</el-table-column>
			</el-table>
		...
		<a-modal title="新增公告" :visible="visible" :confirm-loading="confirmLoading" @ok="handleOk" @cancel="applyhandleCancel">
		...
		<a-modal title="修改公告" :visible="visible1" :confirm-loading="confirmLoading1" @ok="handleOk1" @cancel="applyhandleCancel">
	...
	export default {
		...
		methods: {
			...
			handleOk1(e) {
				let data = {}
					...
					if (valid) {
						...
						let res = await DoUpdateMClubNews(data)
						if (res.data.code === 0) {
							this.$message.success(res.data.msg)
							this.$emit('closeFun')
							this.visible1 = false
			...
			async GetOtherList(){
				...
				data.clubID=this.ID
				// debugger
				let res = await GetMClubNewsList(data);
			...
			deletemember(text) {
				...
				this.$confirm({
					...
					async onOk() {
						let data = {}
						data.id = text.ID
						let res = await DoDeleteMClubNews(data);
						if(res.data.code==0){
							that.GetOtherList()
