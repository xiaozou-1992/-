...
import { GetClubProjectTeacherListManage,DeleteClubProjectTeacher } from '@/api/follow';
const data = [];
export default {
	components: {
		add
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetClubProjectTeacherListManage(data);
			this.loading = false;
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteClubProjectTeacher(data);
					if(res.data.code==0){
						that.getList()
