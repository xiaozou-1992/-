	...
	<div>
		...
		<div>
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				<el-table-column prop="TypeName" label="申请类型" min-width="100"></el-table-column>
		...
		<add :text="text" v-show="addIf" :FlowTypeList="FlowTypeList" @closeFun="closeFun"></add>
	</div>
</template>
...
import { GetApplyItemFlowListManage,DeleteApplyItemFlow,GetFlowTypeList,DoSortApplyItemFlow } from '@/api/follow';
const data = [];
export default {
	components: {
		add
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetApplyItemFlowListManage(data);
			this.loading = false;
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteApplyItemFlow(data);
					if(res.data.code==0){
						that.getList()
		...
		async sortList(text,type){
			...
			data.id = text.ID
			data.type = type
			let res = await DoSortApplyItemFlow(data);
			if(res.data.code==0){
				this.getList();
