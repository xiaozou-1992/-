<template>
	<div class="aid">
		<div class="base">
			<p class="fixed-top">
				{{ JSON.stringify(text) == '{}' ? '添加俱乐部成员' : '修改俱乐部成员' }}
			...
			<div class="main" id="new_message">
				<a-form-model ref="ruleForm" :model="form" :rules="rules">
					...
					<a-form-model-item label="成员学号" prop="UserNo">
...
<script>
import { AddClubMember,UpdateClubMember,getClubPageList,GetUserList } from '@/api/follow';
export default {
	props: {
		text: Object,
		ClubID:String
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddClubMember(data);
					} else {
						res = await UpdateClubMember(data);
					}
					if (res.data.code === 0) {
