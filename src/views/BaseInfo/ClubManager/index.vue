	...
	<div>
		...
		<div>
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				...
				<el-table-column prop="IsAgreeManageMember" label="管理成员权限" min-width="100">
					<template slot-scope="scope">
						<a-tag :color="scope.row.IsAgreeManageMember ? 'green' : 'pink'">{{ scope.row.IsAgreeManageMember?'有':'无' }}</a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="IsAgreeApplyActivity" label="活动申请权限" min-width="100">
					<template slot-scope="scope">
						<a-tag :color="scope.row.IsAgreeApplyActivity ? 'green' : 'pink'">{{ scope.row.IsAgreeApplyActivity?'有':'无' }}</a-tag>
					</template>
				</el-table-column>
...
import { GetClubManagerListManage,DeleteClubManager } from '@/api/follow';
const data = [];
export default {
	components: {
		add
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetClubManagerListManage(data);
			this.loading = false;
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteClubManager(data);
					if(res.data.code==0){
						that.getList()
