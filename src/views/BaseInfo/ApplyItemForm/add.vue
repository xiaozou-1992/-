<template>
	<div class="aid">
		<div class="base">
			<p class="fixed-top">
				{{ JSON.stringify(text) == '{}' ? '添加申请项表单' : '修改申请项表单' }}
...
<script>
import { AddApplyItemForm,UpdateApplyItemForm } from '@/api/follow';
export default {
	props: {
		text: Object,
		FieldTypeList:Array
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddApplyItemForm(data);
					} else {
						res = await UpdateApplyItemForm(data);
					}
					if (res.data.code === 0) {
