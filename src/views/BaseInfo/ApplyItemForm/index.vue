<template>
	<div>
			...
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				...
				<el-table-column prop="FieldType" label="字段类型" min-width="100">
					<template slot-scope="scope">
						<a-tag color="blue">{{ FieldTypeList.filter(item => item.Key == scope.row.FieldType).length>0?FieldTypeList.filter(item => item.Key == scope.row.FieldType)[0].Value:'' }}</a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="IsMust" label="是否必填" min-width="100">
					<template slot-scope="scope">
						<a-tag :color="scope.row.IsMust ? 'green' : 'pink'">{{ scope.row.IsMust?'必填':'非必填' }}</a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="Regular" label="正则验证" min-width="100"></el-table-column>
				<el-table-column prop="ErroeMsg" label="验证失败提示" min-width="100"></el-table-column>
		...
		<add :text="text" v-show="addIf" :FieldTypeList="FieldTypeList" @closeFun="closeFun"></add>
	</div>
</template>
...
import { GetApplyItemFormListManage,DeleteApplyItemForm,GetApplyFormFieldTypeList,DoSortApplyItemForm } from '@/api/follow';
const data = [];
export default {
	components: {
		add
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetApplyItemFormListManage(data);
			this.loading = false;
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteApplyItemForm(data);
					if(res.data.code==0){
						that.getList()
		...
		async sortList(text,type){
			...
			data.id = text.ID
			data.type = type
			let res = await DoSortApplyItemForm(data);
			if(res.data.code==0){
				this.getList();
