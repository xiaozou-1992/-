	...
	<div>
		...
		<div>
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				<el-table-column prop="Name" label="类型名称" min-width="100"></el-table-column>
				<el-table-column prop="Level" label="等级" min-width="100">
					<template slot-scope="scope">
						<a-tag :color="scope.row.Level == 1 ? 'green' : 'pink'">{{ scope.row.Level == 1 ? '校级' : '院级' }}</a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="IsJoin" label="是否允许加入" min-width="200">
					<template slot-scope="scope">
						<a-tag :color="scope.row.IsJoin ? 'green' : 'pink'">{{ scope.row.IsJoin ? '允许' : '不允许' }}</a-tag>
					</template>
				</el-table-column>
				<el-table-column label="操作" width="140" fixed="right">
					<template slot-scope="scope">
						...
						<i title="审批设置" class="iconfont icon-shenpi" style="color: green;" @click="setFlow(scope.row)"></i>
					</template>
				</el-table-column>
			</el-table>
		</div>
...
import { GetClubTypeListManage,DeleteClubType } from '@/api/follow';
const data = [];
export default {
	components: {
		add
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetClubTypeListManage(data);
			this.loading = false;
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteClubType(data);
					if(res.data.code==0){
						that.getList()
