<template>
	<div class="aid">
		<div class="base">
			<p class="fixed-top">
				{{ JSON.stringify(text) == '{}' ? '添加俱乐部类型' : '修改俱乐部类型' }}
			...
			<div class="main" id="new_message">
				<a-form-model ref="ruleForm" :model="form" :rules="rules">
					...
					<a-form-model-item label="级别" prop="Level">
						<a-radio-group name="radioGroup" v-model="form.Level">
							<a-radio :value="1">
								校级
							</a-radio>
							<a-radio :value="2">
					...
					<a-form-model-item label="允许加入" prop="IsJoin">
						<a-radio-group name="radioGroup" v-model="form.IsJoin">
							<a-radio :value="true">
								允许加入
							</a-radio>
							<a-radio :value="false">
...
<script>
import { AddClubType,UpdateClubType } from '@/api/follow';
export default {
	props: {
		text: Object,
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddClubType(data);
					} else {
						res = await UpdateClubType(data);
					}
					if (res.data.code === 0) {
