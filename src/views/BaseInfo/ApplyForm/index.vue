<template>
	<div>
		...
		<div class="option"><a-button type="primary" class="btn" @click="addWorker">新增</a-button></div>
		<div>
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				<el-table-column prop="Key" label="序号" min-width="100"></el-table-column>
				<el-table-column prop="Value" label="名称" min-width="100"></el-table-column>
...
import { GetApplyFormTypeList,GetApplyFormFieldTypeList } from '@/api/follow';
const data = [];
export default {
	components: {
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetApplyFormTypeList()
			this.loading = false;
