	...
	<div>
		...
		<div>
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				<el-table-column prop="Name" label="申请项名称" min-width="100"></el-table-column>
				<el-table-column prop="TypeName" label="类型" min-width="160"></el-table-column>
				<el-table-column prop="Score" label="分值" min-width="100">
					<template slot-scope="scope">
						<a-tag color="green">{{ scope.row.Score }}</a-tag>
					</template>
				</el-table-column>
...
import { GetApplyItemNodeListManage,DeleteApplyItemNode } from '@/api/follow';
const data = [];
export default {
	components: {
		add
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetApplyItemNodeListManage(data);
			this.loading = false;
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteApplyItemNode(data);
					if(res.data.code==0){
						that.getList()
