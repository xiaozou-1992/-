<template>
	<div class="aid">
		<div class="base">
			<p class="fixed-top">
				{{ JSON.stringify(text) == '{}' ? '添加积分申请类型' : '修改积分申请类型' }}
			...
			<div class="main" id="new_message">
				<a-form-model ref="ruleForm" :model="form" :rules="rules">
					...
					<a-form-model-item label="上级申请类型" prop="ParentID">
...
<script>
import { AddApplyItemType,UpdateApplyItemType } from '@/api/follow';
export default {
	props: {
		text: Object,
		ParentList: Array
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddApplyItemType(data);
					} else {
						res = await UpdateApplyItemType(data);
					}
					if (res.data.code === 0) {
