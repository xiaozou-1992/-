<template>
	<div class="aid">
		<div class="base">
			<p class="fixed-top">
				{{ JSON.stringify(text) == '{}' ? '添加积分申请项' : '修改积分申请项' }}
			...
			<div class="main" id="new_message">
				<a-form-model ref="ruleForm" :model="form" :rules="rules">
					...
					<a-form-model-item label="类型" prop="TypeID">
						<a-select optionFilterProp="children" showSearch v-model="form.TypeID">
						  <template v-for="(item, index) in ApplyItemNodeList">
						    <a-select-option :key="item.ID">{{ item.Name }}</a-select-option>
						  </template>
						</a-select>
					</a-form-model-item>
					<a-form-model-item label="分值" prop="Score">
						<a-input type="number" v-model="form.Score" />
					</a-form-model-item>
...
<script>
import { AddApplyItemNode,UpdateApplyItemNode,GetApplyItemTypeList } from '@/api/follow';
export default {
	props: {
		text: Object,
		departmentCVList: Array,
		chargerList: Array
	},
	watch: {
		text: function(text) {
	...
	methods: {
		async getOtherList(){
			let res = await GetApplyItemTypeList()
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddApplyItemNode(data);
					} else {
						res = await UpdateApplyItemNode(data);
					}
					if (res.data.code === 0) {
