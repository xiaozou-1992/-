<template>
	<div class="aid">
		<div class="base">
			<p class="fixed-top">
				{{ JSON.stringify(text) == '{}' ? '添加俱乐部项目' : '修改俱乐部项目' }}
...
<script>
import { AddClubProject,UpdateClubProject } from '@/api/follow';
export default {
	props: {
		text: Object
	},
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddClubProject(data);
					} else {
						res = await UpdateClubProject(data);
					}
					if (res.data.code === 0) {
