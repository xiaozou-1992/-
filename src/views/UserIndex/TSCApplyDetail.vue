				...
				<div class="main" :style="{ height: layoutContentHeight }">
					<a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }" @submit="handleSubmit">
						...
						<a-form-item label="活动图标">
							<a-upload disabled v-decorator="['Ico', { rules: [{ required: true, message: '活动图标不能为空' }] }]" accept="image/*"
							 action="http://api.jlb.ac.acxk.net/Common/DoUploadFile?type=file" list-type="picture-card" :file-list="icofileList"
							 @preview="handlePreview" @change="e => icohandleChange(e, 'Ico')">
						...
						<a-form-item label="开始时间">
							<a-date-picker disabled format="YYYY-MM-DD HH:mm:ss" @change="e => dataChange(e, 'StartTime', 'data')"
						...
						<a-form-item v-for="item in data" :label="item.Name">
							<a-input v-if="item.FieldType == 1" disabled v-decorator="[
									item.ID,
							...
							<a-input disabled v-if="item.FieldType == 2" type="textarea" v-decorator="[
									item.ID,
									{ rules: [{ required: item.IsMust, message: item.ErroeMsg ? item.ErroeMsg : item.Name + '不能为空' }, { validator: rules[item.ID] ? rules[item.ID].validator : '' }] }
								]" />
							...
							<a-input disabled v-if="item.FieldType == 4" type="number" v-decorator="[
									item.ID,
									{ rules: [{ required: item.IsMust, message: item.ErroeMsg ? item.ErroeMsg : item.Name + '不能为空' }, { validator: rules[item.ID] ? rules[item.ID].validator : '' }] }
								]" />
							<!-- {{ JSON.stringify(rules) }} -->
							<a-upload disabled v-if="item.FieldType == 5 || item.FieldType == 6" v-decorator="[item.ID, { rules: [{ required: item.IsMust, message: item.ErroeMsg ? item.ErroeMsg : item.Name + '不能为空' }] }]"
							...
							<a-date-picker disabled v-if="item.FieldType == 7" @change="e => dataChange(e, item.ID, 'data')" v-decorator="[item.ID, { rules: [{ required: item.IsMust, message: item.ErroeMsg ? item.ErroeMsg : item.Name + '不能为空' }] }]" />
							<a-time-picker disabled v-if="item.FieldType == 8" @change="e => dataChange(e, item.ID, 'time')" v-decorator="[item.ID, { rules: [{ required: item.IsMust, message: item.ErroeMsg ? item.ErroeMsg : item.Name + '不能为空' }] }]" />
						</a-form-item>
						<a-form-item v-if="!text.IsAdopt" class="fixed-bottom" :wrapper-col="{ span: 12, offset: 5 }">
							<a-button type="primary" @click="apply">审批</a-button>
						</a-form-item>
					</a-form>
				</div>
				<a-modal :visible="previewVisible" :footer="null" @cancel="handleCancel"><img alt="example" style="width: 100%"
					 :src="previewImage" /></a-modal>
				<a-modal title="审批" :visible="visible" :confirm-loading="confirmLoading" @ok="handleOk" @cancel="applyhandleCancel">
					<a-form-model ref="ruleForm" :model="form1" :rules="rules1" :label-col="labelCol" :wrapper-col="wrapperCol">
						...
						<a-form-model-item label="是否通过" prop="isAdopt">
							<a-select :allowClear="true" v-model="form1.isAdopt">
						...
						<a-form-model-item label="批复" prop="comment">
							<a-input v-model="form1.comment" type="textarea" />
						</a-form-model-item>
...
<script>
	...
	import {
		UserAllActivityFlowDoGetDetail,
		UserUpdateActivity,
	...
	export default {
		...
		methods: {
			 ...
			 handleOk(e) {
				let data = {}
					...
					if (valid) {
						...
						data.isAdopt = this.form1.isAdopt
						data.comment = this.form1.comment
			...
			async getList() {
				...
				let data = {};
				data.id = this.text.ID;
				let res = await UserAllActivityFlowDoGetDetail(data);
				if (res.data.code == 0) {
								...
								for (let m = 0; m < arr.length; m++) {
									if (arr[m]) {
										that.fileList[i].push({
											uid: m,
											url: arr[m].indexOf("http") >= 0 ? arr[m] : 'http://api.jlb.ac.acxk.net' + arr[m],
											name: ' ',
											status: 'done'
										});
									}
