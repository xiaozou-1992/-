...
import { DoAddUserApplyItem,DoGetFormListUserApplyItem,GetApplyItemNodeList,GetApplyItemTypeList } from '@/api/follow';
const data = [];
export default {
	components: {
	},
	...
	methods: {
		moment,
		// TimeGet(){
		// 	setTimeout(this. getOtherList(), 1000); 
		// },
		async getOtherList(){
			this.data = [];
			this.loading = true;
		...
		handleSubmit(e) {
			...
			this.form.validateFields(async (err, values) => {
				...
				if (!err) {
					...
					let res = await DoAddUserApplyItem(data);
					if(res.data.code==0){
