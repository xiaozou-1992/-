<template>
	<div class="all" v-loading="loading">
		...
		<detail  :time="time" :text="text"  v-show="showModal" @closeFun="closeFun(1)"></detail>
		<look  :time="time" :text="looktext"  v-show="detailshowModal" @closeFun="closeFun(1)"></look>
	</div>
</template>
...
<script>
	...
	import {
		UserAllTeacherExamPageList
	} from '@/api/follow';
	import detail from './TSCApply'
	import look from './TSCApplyDetail.vue'
	const data = [];
	export default {
		components: {
			detail,
		...
		methods: {
			...
			async getList() {
				...
				let res = await UserAllTeacherExamPageList(data);
				this.loading = false;
