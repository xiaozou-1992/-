<template>
	<div class="all" v-loading="loading">
				...
				<a-row :gutter="24">
					...
					<a-col :span="8">
						<a-form-item label="状态">
							<a-select :allowClear="true" v-decorator="['isAgree',{initialValue:'null'}]" @change="handleChange" placeholder="全部" optionFilterProp="children"
							 showSearch>
		...
		<div class="option">
			<!-- <a-button type="primary" class="btn" disabled @click="addList">批量审核</a-button> -->
		...
		<applyJoin :approvalType="1" :text="text" v-show="showModal" @closeFun="closeFun(1)"></applyJoin>
		<batchApply :approvalType="1" :text="batchtext" v-show="batchshowModal" @closeFun="closeFun"></batchApply>
		<a-modal
		      title="审核结果"
		      :visible="visible"
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				let res = await GetUserClubApplyPageList(data);
				this.loading = false;
