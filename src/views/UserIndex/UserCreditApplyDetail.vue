				...
				<div class="main" :style="{ height: layoutContentHeight }">
					...
					<a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }" @submit="handleSubmit">
						<a-form-item label="申请项">
							<a-select disabled :allowClear="true" v-decorator="['ApplyItemNodeID', { rules: [{ required: true, message: '申请项不能为空' }] }]"
						...
						<a-form-item label="附件">
							<a-upload disabled v-decorator="['Files', { rules: [{ required: true, message: '活动图标不能为空' }] }]" accept="image/*"
...
<script>
	...
	export default {
		...
		methods: {
			moment,
			async getOtherList() {
				let res = await DoGetFormListUserApplyItem()
				this.data = res.data.data
				let res1 = await GetApplyItemNodeList()
				this.applyItemNodeList = res1.data.data
			},
			applyHander() {
