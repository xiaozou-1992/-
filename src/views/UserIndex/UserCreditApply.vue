	...
	<div class="all" v-loading="loading">
		...
		<div>
			<el-table ref="singleTable" :data="data" border highlight-current-row style="width: 100%">
				<el-table-column prop="Title" label="标题" min-width="100" fixed="left"></el-table-column>
				<!-- <el-table-column prop="AddUserNo" label="学号" min-width="100" ></el-table-column> -->
				<el-table-column prop="ApplyItemNodeName" label="申请项" min-width="120"></el-table-column>
				<el-table-column prop="IsManager" label="状态" min-width="100">
					<template slot-scope="scope">
						<!-- {{ scope.row.IsManager==true?'管理员':'成员' }} -->
						<a-tag color="green" v-if="scope.row.State==3"> {{scope.row.StateName}} </a-tag>
						<a-tag color="pink" v-else> {{scope.row.StateName}} </a-tag>
					</template>
				</el-table-column> 
				<el-table-column pr0.o3p="AddTime" label="时间" min-width="100">
					<template slot-scope="scope">
						{{ scope.row.AddTime }}
						</template>
				</el-table-column>
				<el-table-column prop="IsEnable" label="操作" min-width="100" fixed="right">
					<template slot-scope="scope">
						...
						<i title="修改" v-if="scope.row.State==1"  class="iconfont icon-editor1" style="color: #1890FF;"
						 @click="update(scope.row)"></i>
						 <i title="提交审批" class="iconfont icon-tijiao" v-if="scope.row.State == 1" style="color: #1890FF;" @click="sumbitapply(scope.row,'manage')"></i>
						 <i title="删除" v-if="!scope.row.IsManager&&scope.row.State==1" class="iconfont icon-delete1" style="color: red;" @click="deletemember(scope.row)"></i>
					</template>
				</el-table-column>
			</el-table>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				let res = await UserApplyItemPageList(data);
				this.loading = false;
