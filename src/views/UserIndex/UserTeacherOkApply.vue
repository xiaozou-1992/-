<template>
	<div class="all" v-loading="loading">
		<div id="components-form-demo-advanced-search">
			<div class="h2">指导老师已审批</div>
		</div>
		<div class="option">
		</div>
		<div>
			<el-table ref="singleTable" :data="data" border highlight-current-row style="width: 100%">
				<el-table-column prop="ActivityTitle" label="名称" min-width="100" fixed="left"></el-table-column>
				<el-table-column prop="IsManager" label="类型" min-width="100">
					<template slot-scope="scope">
						<a-tag color="green"> {{scope.row.FlowTypeName}} </a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="IsAdopt" label="是否通过" min-width="100">
					<template slot-scope="scope">
						<a-tag color="green" v-if="scope.row.IsAdopt"> 通过 </a-tag>
						<a-tag color="pink" v-else> 未通过 </a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="AddTime" label="审批时间" min-width="100"></el-table-column>
				<el-table-column prop="ExamComment" label="审批内容" min-width="100"></el-table-column>
				<el-table-column prop="IsEnable" label="操作" min-width="100" fixed="right">
					<template slot-scope="scope">
						<i title="查看"  class="iconfont icon-chakan" style="color: #1890FF;"
		...
		<detail  :time="time" :text="text" :ID="ClubID" :linktype="linktype" v-show="showModal" @closeFun="closeFun(1)"></detail>
	</div>
</template>
...
<script>
	...
	import {
		UserAllTeacherAlreadyExamPageList
	} from '@/api/follow';
	import detail from './TSCApplyDetail'
	const data = [];
	export default {
		components: {
			detail
		...
		methods: {
			...
			async getList() {
				...
				let res = await UserAllTeacherAlreadyExamPageList(data);
				this.loading = false;
