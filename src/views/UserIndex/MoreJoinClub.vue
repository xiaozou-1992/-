<template>
	<a-layout id="components-layout-demo-top" class="layout">
		<a-layout-content style="width: 100%;">
			<div :style="{  minHeight: '380px' }" v-loading="loading">
				<div class="c-content">
					<div class="content-left">
						<a-card title="已加入的俱乐部" :bordered="false" style="width: 100%" class="acard">
							<!-- <span class="joinclub" slot="extra">加入更多俱乐部</span> -->
							<a-button type="primary" slot="extra" @click="joinclub">
								加入俱乐部
							</a-button>
							<div class="content-list" v-for="(item,index) in ClubList" :key="index">
								<div class="cimg">
									<a-avatar :src="item.Ico==null?'':(item.Ico.indexOf('http')!=-1?item.Ico:''+item.Ico)" />
								</div>
								<div class="cdetail">
									<span class="cd-name" :title="item.Title"> {{ item.Name }} </span>
									...
									<span class="cd-time"><a-tag  color="blue" @click="JoinActivity(item)">加入活动</a-tag><a-tag color="blue" @click="Notice(item)">
												公告 </a-tag><a-tag  color="pink" @click="ExitClub(item)">退出</a-tag></span>
								</div>
							</div>
							<div class="content-list" v-if="ClubList.length<=0" style="width: 100%;text-align: center;font-size: 16px;background-color: white;display: block;">暂无已加入的俱乐部</div>
						</a-card>
					</div>
				</div>
			</div>
