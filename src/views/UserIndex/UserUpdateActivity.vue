		...
		<div class="main" :style="{ height: layoutContentHeight }">
			<a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }" @submit="handleSubmit">
				...
				<a-form-item label="活动图标">
					<a-upload v-decorator="['Ico', { rules: [{ required: true, message: '活动图标不能为空' }] }]" accept="image/*" action="http://api.jlb.ac.acxk.net/Common/DoUploadFile?type=file"
				...
				<a-form-item label="开始时间">
					<a-date-picker :show-time="{ defaultValue: moment('00:00:00', 'HH:mm:ss') }" format="YYYY-MM-DD HH:mm:ss" @change="e => dataChange(e, 'StartTime', 'data')" v-decorator="['StartTime', { rules: [{ required: true, message: '活动开始时间不能为空' }] }]" />
				</a-form-item>
				<a-form-item label="结束时间">
					<a-date-picker :show-time="{ defaultValue: moment('00:00:00', 'HH:mm:ss') }" format="YYYY-MM-DD HH:mm:ss" @change="e => dataChange(e, 'EndTime', 'data')" v-decorator="['EndTime', { rules: [{ required: true, message: '活动结束时间不能为空' }] }]" />
				</a-form-item>
