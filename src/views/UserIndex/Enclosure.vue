	...
	<div class="all"  v-loading="loading">
		...
		<div>
			<el-table ref="singleTable" :data="data" border highlight-current-row style="width: 100%">
				<!-- <el-table-column prop="AddUserName" label="姓名" min-width="100" fixed="left"></el-table-column> -->
				...
				<el-table-column prop="IsEnable" label="操作" min-width="100" fixed="right">
					<template slot-scope="scope">
						<a-icon title="下载"  type="download" style="color: #1890FF;font-size: 20px;" @click="look(scope.row)" />
						<!-- <i title="下载"  class="iconfont icon-chakan" style="color: #1890FF;" @click="look(scope.row)"></i> -->
						<i v-if="IsManager||ChairmanNo==thisChairmanNo" title="删除" class="iconfont icon-delete1" style="color: red;" @click="deletemember(scope.row)"></i>
					</template>
				</el-table-column>
			</el-table>
		...
		<add  :ID="ID"  v-show="addIf" @closeFun="closeFun"></add>
...
<script>
	...
	import {
		UserActivityFile,DownUserActivityFile,DeleteUserActivityFile,UserActivityDetail
