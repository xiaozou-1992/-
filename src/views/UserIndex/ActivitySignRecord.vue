	...
	<div class="all"  v-loading="loading">
		...
		<div>
			<el-table ref="singleTable" :data="data" border highlight-current-row style="width: 100%">
				...
				<el-table-column prop="AddUserNo" label="是否签到" min-width="100" >
					<template slot-scope="scope">
						<a-tag color="green" v-if="scope.row.IsSign==true"> 已签到 </a-tag>
						<a-tag color="pink" v-else> 未签到 </a-tag> 
						</template>
				</el-table-column>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				let res = await UserDoGetSignList(data);
				this.loading = false;
