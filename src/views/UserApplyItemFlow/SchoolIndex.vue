<template>
	<div>
		<div class="h2">积分申请审批</div>
		<a-tabs default-active-key="3" @change="tabChange">
		      <a-tab-pane key="3" tab="学校待审批">
		        <div id="components-form-demo-advanced-search">
		        	<a-form class="ant-advanced-search-form home-form" :form="form" @submit="handleSearch2">
		        		<a-row :gutter="24">
		        			<a-col :span="8">
		        			...
		        			<a-col :span="8" style="margin-top:4px;">
		        				...
		        				<a-button :style="{ marginLeft: '8px' }" @click="handleReset2" class="btn2">重置</a-button>
		        			</a-col>
		        		</a-row>
		        	</a-form>
		        </div>
		        <div>
		        	<el-table :data="data2" v-loading="loading2" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
...
import { GetUserApplyItemFlowSchoolExamPageList,GetUserApplyItemFlowSchoolAlreadyExamPageList, DeleteActivity,GetActivityStateList,GetClubPageListCommon } from '@/api/follow';
const data2 = [];
const data3 = [];
export default {
	components: {
	...
	methods: {
		...
		async getList2() {
			...
			data.pageIndex = this.pagination2.currentPage;
			data.pageSize = this.pagination2.pageSize;
			let res = await GetUserApplyItemFlowSchoolExamPageList(data);
			this.loading2 = false;
		...
		async getList3() {
			...
			let res = await GetUserApplyItemFlowSchoolAlreadyExamPageList(data);
			this.loading3 = false;
		...
		async getOtherList(){
			...
			let res1 = await GetClubPageListCommon(data);
			this.ClubList = res1.data.data
		},
		addList() {
			this.text = {};
		...
		seeList(text,name,type){
			let path = 'UserApplyItemFlowDetail'
			this.$router.push({
				name: path,
				query: {
					ID: text.ID,
