	...
	<div>
		...
		<div>
			<el-table :data="data" v-loading="loading" border :max-height="tableHeight" highlight-current-row style="width: 100%;">
				<el-table-column prop="Name" label="活动类型名称" min-width="100"></el-table-column>
				<el-table-column prop="Score" label="分值" min-width="100">
					<template slot-scope="scope">
						<a-tag color="green">{{ scope.row.Score?scope.row.Score:'暂无' }}</a-tag>
					</template>
				</el-table-column>
				<el-table-column prop="AddTime" label="添加时间" min-width="200"></el-table-column>
...
import { getActivityTypeList,DeleteActivityType } from '@/api/follow';
const data = [];
export default {
	components: {
		add
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DeleteActivityType(data);
					if(res.data.code==0){
						that.getList()
