<template>
	<div>
		<div class="h2">
			活动详情
			<a-button v-if="flowType!=2" style="float: right;margin-top: 4px;" type="primary" @click="applyHander()">审批</a-button>
		</div>
		<div class="main" :style="{ height: layoutContentHeight }">
			...
			<a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }" @submit="handleSubmit">
				...
				<a-form-item label="开始时间">
					<a-date-picker disabled show-time  @change="e => dataChange(e, 'StartTime', 'data')" v-decorator="['StartTime', { rules: [{ required: true, message: '活动开始时间不能为空' }] }]" />
				</a-form-item>
				<a-form-item label="结束时间">
					<a-date-picker disabled show-time @change="e => dataChange(e, 'EndTime', 'data')" v-decorator="['EndTime', { rules: [{ required: true, message: '活动结束时间不能为空' }] }]" />
				</a-form-item>
				<a-form-item label="审核设置">
					<a-radio-group disabled v-decorator="['ApplyIsNeedExam', { rules: [{ required: true, message: '审核设置不能为空' }] }]">
...
import { ActivityFlowGetDetail } from '@/api/follow';
