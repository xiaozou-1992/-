		...
		<div class="base">
			...
			<div class="main" id="new_message">
				<a-form-model ref="ruleForm" :model="form" :rules="rules">
					...
					<a-form-model-item label="编码" prop="Code"><a-input v-model="form.Code" /></a-form-model-item>
					<a-form-model-item label="上级部门" prop="ParentCode">
						<a-select v-model="form.ParentCode" style="width:100%">
						  <a-select-option
...
<script>
import { AddDepart,UpdateDepart } from '@/api/follow';
export default {
	props: {
		text: Object,
		ParentList: Array
			...
			this.$refs.ruleForm.validate(async valid => {
				if (valid) {
					...
					if (JSON.stringify(this.text) == '{}') {
						res = await AddDepart(data);
					} else {
						res = await UpdateDepart(data);
					}
					if (res.data.code === 0) {
