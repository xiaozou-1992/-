<template>
  <div class="aid">
    <div class="base">
      <p class="fixed-top">
        {{ !text.ID ? '添加活动类型' : '修改活动类型' }}
        <a-icon @click="closeFunction" :style="{ fontSize: '20px',float: 'right', margin: '10px' }" type="close-circle" />
      </p>
      <div class="main" id="new_message">
        <a-form-model ref="ruleForm" :model="form" :rules="rules">
          <a-form-model-item label="活动类型" prop="Name">
            <a-input v-model="form.Name" :allowClear="true" placeholder="请输入活动类型名称" />
          </a-form-model-item>
			...
			handleSubmit(e) {
				this.$refs.ruleForm.validate(async valid => {
					if (valid) {
						let res = {}
						if (!this.text.ID) {
							res = await DoAddActType(this.form)
						} else {
							res = await DoUpdateActType(this.form)
						}
						if (res.data.code === 0) {
