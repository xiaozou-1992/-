  ...
  <div class="content">
    ...
    <div id="components-form-demo-advanced-search">
      <a-form class="ant-advanced-search-form home-form" :form="form" @submit="handleSearch">
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item label="类型名称"><a-input class="field-right" placeholder="请输入类型名称" v-decorator="[`name`]" autocomplete="off" /></a-form-item>
          </a-col>
    ...
    <div style="margin-top: 20px;">
      ...
      >
        ...
        <el-table-column label="操作" width="88" fixed="right">
          <template slot-scope="scope">
            <a-popover title="修改">
              <i class="el-font el-icon-edit-outline" style="color: #1890FF;" @click="addList(scope.row)"></i>
            </a-popover>
            <a-popover title="删除">
...
export default {
	...
	methods: {
		...
		async getList() {
			...
			let res = await GetActTypePageList(data)
			this.loading = false
		...
		deleteList(text) {
			...
			this.$confirm({
				...
				async onOk() {
					let data = {}
					data.id = text.ID
					let res = await DoDeleteActType(data)
					if (res.data.code === 0) {
						that.getList()
