      ...
      <a-form-model class="left" ref="ruleForm" :model="form" :rules="rules">
		...
		<a-form-model-item label="">
         <template @click="tablerow">
         	<!-- <el-table　highlight-current-row :data="tableData"  height="400" v-loading="loading" :row-class-name="tableRowClassName"  @row-click="rowClick" tooltip-effect="dark" border style="width: 100%">
         		<el-table-column label="选择" fixed="left" width="60">
         		...
         		<el-table-column prop="ParentCommodityName" width="270" label="商品名称或名称">
         			<template slot-scope="scope">
         				<a-tag v-if="scope.row.Title" color="green">{{ scope.row.TypeName }}</a-tag>
         				<a-tag v-if="scope.row.Title" color="cyan">{{ scope.row.Specifications }}</a-tag>
         				<a-tag v-if="scope.row.Title" color="red">{{ scope.row.StateID==1?'上架中':'已下架' }}</a-tag>
         				<div v-if="scope.row.Title">{{ scope.row.Title }}</div>
						<div v-if="scope.row.Name">{{ scope.row.Name }}</div>
         			</template>
         		</el-table-column>
				<el-table-column prop="linkurl" min-width="100" label="链接">
