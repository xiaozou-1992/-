...
const service1 = axios.create({
    ...
    headers: {
		...
		'Content-type': 'application/json; charset=UTF-8',
		'Type':'3',
		'DealerID':Cache.get('info')&&JSON.stringify(Cache.get('info'))!=='{}'?Cache.get('info').DealerID:'111'
		// 'DealerID':store.state.info&&JSON.stringify(store.state.info)!=='{}'?store.state.info.DealerID:''
        // 'userToken':'gxvXD+urlPx='
    },
    withCredentials: false, // 跨域请求时是否需要使用凭证
    maxContentLength: 2000, // 响应内容的最大尺寸
...
const fetch = async function (query = {}) {
    ...
    try {
		if(query.apiType){
			res = await service1(query)
			let {config: {_service1, _sign}} = res
			delete _service1[_sign]
			delete res.config._service1
			return res
		}else{
